...
onMounted(() => {
  ...
  if (import.meta.env.VITE_OPEN_WEATHER_API_KEY === undefined) {
    ...
    openweathermapApiKey.value = 'b3aaa0b3323c0baab93aff38f75b44cb'
...
const searchCity = async (inputCity) => {
  ...
  try {
    const response = await axios.get('https://api.openweathermap.org/geo/1.0/direct?q=' + inputCity + '&APPID=' + 'b3aaa0b3323c0baab93aff38f75b44cb'
)
  ...
  if (city !== '') {
    ...
    try {
      const response2 = await axios.get('https://api.openweathermap.org/data/2.5/weather?lat=' + latitude + '&lon=' + longitude + '&units=imperial&APPID=' + 'b3aaa0b3323c0baab93aff38f75b44cb')
      
      const response3 = await axios.get('https://api.openweathermap.org/data/2.5/forecast?lat=' + latitude + '&lon=' + longitude + '&units=imperial&APPID=' + 'b3aaa0b3323c0baab93aff38f75b44cb')
      ...
      const forecast = response3.data.list.map(item => ({
        date: item.dt_txt,
